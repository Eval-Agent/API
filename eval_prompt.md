...
$h(u) = g(u) + f(u)$
where $h(u)$ is the heuristical outcome, $g(u)$ is the cost incurred in the past till the current point, $f(u)$ is the distance b/w the now-state and the future goal.
